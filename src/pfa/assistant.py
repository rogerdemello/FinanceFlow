...
class PersonalFinanceAssistant:
    ...
    def _load_from_db(self) -> None:
        ...
        rows = cur.fetchall()
        self.savings_goals = {
            r[0]: {"amount": float(r[1]), "target_date": r[2]} for r in rows
        }

        cur.execute(
            "SELECT income, expenses_total, recommended_savings FROM budget ORDER BY id DESC LIMIT 1"
        )
    ...
    def create_budget(
        ...
        leftover = round(
            max(0.0, income - monthly_expenses_total - recommended_savings), 2
        )
        ...
        if self.db_path:
            try:
                ...
                cur = conn.cursor()
                cur.execute(
                    "INSERT INTO budget (income, expenses_total, recommended_savings, updated_at) VALUES (?, ?, ?, datetime('now'))",
    ...
    def manage_debt(
        ...
        if self.db_path:
            try:
                ...
                cur = conn.cursor()
                cur.execute(
                    "INSERT OR REPLACE INTO debts (name, balance, interest, minimum) VALUES (?, ?, ?, ?)",
                    (debt_name, balance, interest_rate, minimum_payment),
                )
                conn.commit()
                conn.close()
            except Exception:
    ...
    def debt_payoff_plan(self, method: str = "avalanche") -> List[str]:
        ...
        plan = [
            f"Pay off {name}: ${d['balance']} at {d['interest_rate']}% (min ${d['minimum_payment']}/mo)"
            for name, d in items
        ]
        return plan

    def set_savings_goal(
        self, goal_name: str, amount: float, target_date: str
    ) -> Dict[str, object]:
        ...
        self.savings_goals[goal_name.strip()] = {
            "amount": amount,
        ...
        if self.db_path:
            try:
                ...
                cur = conn.cursor()
                cur.execute(
                    "INSERT OR REPLACE INTO savings_goals (name, amount, target_date) VALUES (?, ?, ?)",
                    (goal_name, amount, target_date),
                )
                conn.commit()
                conn.close()
            except Exception:
    ...
    def savings_progress(self) -> List[str]:
        ...
        return [
            f"Goal: {k} — Target: ${v['amount']} by {v['target_date']}"
            for k, v in self.savings_goals.items()
        ]
